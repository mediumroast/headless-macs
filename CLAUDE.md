...
This repo is public/open-source. Never write a real account username,
real name, email address, or other personally-identifying detail into
anything that gets committed — `PHASE_N_PLAN.md` files, `CHANGELOG.md`,
README/docs, code comments, or commit messages. This applies even when
quoting real diagnostic output gathered while debugging (e.g. `$HOME`
resolving to `/Users/<realname>` in a live-tested example) — genericize it
(`/Users/<operator>`, `~<operator>`, `example-user`) rather than
transcribing the literal value; the substance of a finding almost never
depends on the specific username involved. If a session accidentally
introduces one on an unmerged branch, fix it there directly; if it's
already merged to `main`, flag it to the user rather than silently
